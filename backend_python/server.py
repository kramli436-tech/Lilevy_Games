...
# =========================================================================
ONLINE_CLIENTS = {}  # { session_id: { "id": sid, "name": ..., "last_seen": timestamp } }

def prune_expired_presence():
    now = time.time()
    expired = [sid for sid, data in list(ONLINE_CLIENTS.items()) if now - data.get("last_seen", 0) > 18]
    for sid in expired:
        if sid in ONLINE_CLIENTS:
            del ONLINE_CLIENTS[sid]

def record_presence(session_id, player_name="Pemain", view="home", room_id=None):
    prune_expired_presence()
    ONLINE_CLIENTS[session_id] = {
        "id": session_id,
        "name": player_name,
        "view": view,
        "roomId": room_id,
...
def remove_presence(session_id):
    if session_id in ONLINE_CLIENTS:
        del ONLINE_CLIENTS[session_id]
    prune_expired_presence()
    return len(ONLINE_CLIENTS)
...
class LilevyApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 4. Endpoint Realtime Online Presence
        elif path == "/api/presence/online":
            prune_expired_presence()
            self._respond_json({
                "success": True,
                "onlineCount": max(1, len(ONLINE_CLIENTS)),
                "activePlayers": list(ONLINE_CLIENTS.values())
            })
            return
    ...
    def do_POST(self):
        ...
        # 9. Endpoint Heartbeat Presensi Realtime
        elif path == "/api/presence/heartbeat":
            session_id = data.get("sessionId", f"sess_{random.randint(1000, 9999)}")
            player_name = data.get("name", "Pemain")
            view = data.get("view", "home")
            room_id = data.get("roomId", None)
            count = record_presence(session_id, player_name, view, room_id)
            self._respond_json({
                "success": True,
                "onlineCount": count,
        ...
        # 10. Endpoint Leave Presensi Realtime
        elif path == "/api/presence/leave":
            session_id = data.get("sessionId")
            count = remove_presence(session_id) if session_id else len(ONLINE_CLIENTS)
            self._respond_json({
                "success": True,
                "onlineCount": count
            })
