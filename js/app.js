...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Inisialisasi Callback Jaringan Room Realtime
  if (window.roomNetwork) {
    window.roomNetwork.onChatMessage = (chat) => {
      if (window.monopolyChat) window.monopolyChat.receiveRemoteMessage(chat);
    };

    window.roomNetwork.onStateReceived = (gameState) => {
      if (window.monopolyEngine && gameState) {
        window.monopolyEngine.applyState(gameState);
        const mapName = MONOPOLY_MAPS[gameState.currentMapId]?.name || 'Monopoli Dunia Global';
        document.getElementById('mono-title-text').textContent = mapName;
        document.getElementById('mono-game-mode-badge').textContent = `Room [${gameState.roomId}]`;
        window.monopolyUI?.renderBoard();
      }
    };

    window.roomNetwork.onPlayerJoined = (newPlayer) => {
      showToast(`🎉 ${newPlayer.name} (${newPlayer.token}) berhasil masuk ke dalam Room!`);
      window.monopolyUI?.renderBoard();
    };

    window.roomNetwork.onConnectionStatus = (status, msg) => {
      if (msg) showToast(msg);
    };
  }

  // Action Buttons Monopoly dengan Sinkronisasi Jaringan
  document.getElementById('btn-mono-roll')?.addEventListener('click', () => {
    if (window.roomNetwork && !window.roomNetwork.isHost && window.roomNetwork.status === 'CONNECTED') {
      window.roomNetwork.sendAction('ROLL');
    } else {
      window.monopolyEngine.rollDice();
      if (window.roomNetwork?.isHost) window.roomNetwork.broadcastGameState();
    }
  });

  document.getElementById('btn-mono-end-turn')?.addEventListener('click', () => {
    if (window.roomNetwork && !window.roomNetwork.isHost && window.roomNetwork.status === 'CONNECTED') {
      window.roomNetwork.sendAction('END_TURN');
    } else {
      window.monopolyEngine.endTurn();
      if (window.roomNetwork?.isHost) window.roomNetwork.broadcastGameState();
    }
  });
  ...
  // Form Submit: Buat Room Baru (Host Mode)
  formCreate?.addEventListener('submit', (e) => {
    ...
    const hostId = activeUser?.id || ('usr_host_' + Date.now());
    const configs = [];
    ...
    // Inisialisasi Host Jaringan Online Realtime
    if (window.roomNetwork) {
      window.roomNetwork.hostRoom(window.monopolyEngine.roomId, hostPlayer);
    }

    document.getElementById('mono-title-text').textContent = MONOPOLY_MAPS[mapId]?.name || 'Monopoli Pro';
    document.getElementById('mono-game-mode-badge').textContent = mode === 'ai' ? 'Lawan AI' : `Host (${window.monopolyEngine.roomId})`;
    
    closeModal(document.getElementById('modal-mono-setup'));
    window.monopolyUI?.renderBoard();
    showToast(`👑 Room [${window.monopolyEngine.roomId}] Berhasil Dibuat! Bagikan link/kode ke teman Anda.`);
  });

  // Form Submit: Gabung Room Teman (Guest Online Join)
  formJoin?.addEventListener('submit', (e) => {
    ...
    const guestPlayer = {
      id: activeUser?.id || ('usr_guest_' + Date.now() + '_' + Math.floor(Math.random() * 1000)),
      name: joinName,
      avatar: activeUser?.avatar || '👤',
      token: selectedTokenInJoin,
      isAI: false
    };

    // Inisialisasi Jaringan Guest
    if (window.roomNetwork) {
      window.roomNetwork.joinRoom(roomCode, guestPlayer);
    }

    document.getElementById('mono-title-text').textContent = 'Monopoli Dunia Global';
    document.getElementById('mono-game-mode-badge').textContent = `Join Room (${roomCode})`;

    closeModal(document.getElementById('modal-mono-setup'));
    showToast(`🚀 Menghubungkan ke Room [${roomCode}]... Harap tunggu sinkronisasi dengan Host.`);
  });
