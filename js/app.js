...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // =========================================================================
  // 2. GLOBAL CONTROLS (TEMA, SUARA, PROFIL, LEADERBOARD)
  // =========================================================================
  ...
  function updateSoundIcon() {
    if (!soundIcon) return;
    soundIcon.setAttribute('data-lucide', window.soundEngine?.isMuted ? 'volume-x' : 'volume-2');
    if (window.lucide) window.lucide.createIcons();
  }
  updateSoundIcon();
  ...
  function updateHeaderProfile() {
    ...
    if (navName) navName.textContent = user.username || (isLogged ? 'Pemain' : 'Tamu (Guest)');
    if (badgeEl) {
  ...
  // =========================================================================
  // 3. MODAL CONTROLLER HELPERS & TOAST
  // =========================================================================
  ...
  document.getElementById('btn-tts-ai-generate')?.addEventListener('click', async () => {
    ...
    try {
      const resp = await fetch(`http://localhost:8000/api/tts/generate?category=${chosenCat}`);
      if (resp.ok) {
  ...
  document.getElementById('mono-room-badge-btn')?.addEventListener('click', () => {
    ...
    navigator.clipboard?.writeText(code).then(() => {
      showToast(`📋 Kode ID Room [${code}] berhasil disalin ke clipboard!`);
    }).catch(() => {
