...
const Main = (function() {
  ...
  function init() {
    ...
    if (typeof MatchHistory !== 'undefined') MatchHistory.init();
  ...
  function setupMenuEvents() {
    ...
    document.getElementById('btn-menu-archive')?.addEventListener('click', () => { if(typeof MatchHistory !== 'undefined') MatchHistory.showArchiveModal(); });
  ...
  function setupSkillButton() { document.getElementById('btn-skill')?.addEventListener('click', () => {
    if(typeof UI !== 'undefined') UI.showSkillModal();
  }); }
  ...
  function setupHistoryButton() { document.getElementById('btn-history-toggle')?.addEventListener('click', () => { if(typeof MatchHistory !== 'undefined') MatchHistory.showHistoryModal(); }); }
  ...
  function setupNetworkEvents() {
    Events.on('roomJoined', (data) => {
      ...
      // ONLY switch to lobby if game is not in progress!
      const gs = typeof Game !== 'undefined' ? Game.getState() : null;
      if(!gs || gs.phase !== 'PLAYING') {
        showScreen('lobby');
      } else {
        // If game is in progress and we are host, broadcast state so reconnecting clients get current game
        if(Network.getIsHost()) {
          Network.broadcastState(gs);
        }
      }
      if(typeof UI !== 'undefined' && (!gs || gs.phase !== 'PLAYING')) {
        UI.showToast('Room: ' + data.code, 'success');
      }
    });
    ...
    Events.on('stateSync', (newState) => {
      ...
      if(typeof UI !== 'undefined') {
        ...
        // If an active auction is running, auto open auction modal on guest screen
        if(newState.auction && newState.auction.isActive) {
          UI.showAuctionModal();
        } else {
          const auctionModal = document.getElementById('modal-auction');
          if(auctionModal && auctionModal.classList.contains('active') && (!newState.auction || !newState.auction.isActive)) {
  ...
  function handleNetworkAction(action) {
    switch(action.type) {
      ...
      case 'startAuction': Auction.startAuction(action.tileIndex, Game.getState()); break;
      case 'placeBid': Auction.placeBid(action.playerId, action.amount, Game.getState()); break;
      case 'passAuction': Auction.passAuction(action.playerId, Game.getState()); break;
      case 'addBot':
  ...
  function addBot() {
    const botChars = ['banker','engineer','trader','politician','gambler','guardian','hacker','tycoon','cyborg','broker','detective','alchemist'];
    const botTitles = [
      'Nexus-9 [Grandmaster]', 'Quantum-AI [Hardcore]', 'CyberViper [Elite]',
      'Zero-One [Master]', 'NeonTitan [Hardcore]', 'Aegis-Bot [Elite]',
      'GhostRunner [Phantom]', 'Ares-Prime [Megacorp]', 'Spectre-V [Apex]',
      'Chrono-9 [Grandmaster]', 'Oracle-X [Hardcore]', 'Vortex-Alpha [Master]'
    ];
    ...
    const botIndex = lobbyPlayers.filter(p => p.isBot).length;
    const botName = botTitles[botIndex % botTitles.length] || ('CyberBot ' + (botIndex + 1));
