...
class AuthEngine {
  ...
  // Sinkronisasi 1 user ke Backend Python API
  async syncWithBackend(user) {
    try {
      await fetch(`${this.getBackendUrl()}/api/auth/sync`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(user)
      });
    } catch (e) {
      // Backend offline is OK; LocalStorage is permanently persistent
    }
  }

  // Sinkronisasi seluruh database pengguna ke file disk di backend Python
  async syncAllWithBackend() {
    try {
      await fetch(`${this.getBackendUrl()}/api/auth/save-file`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
