const UI = (function() {
  ...
  function updateHUD(gameState) {
    ...
    if(turnEl) turnEl.textContent = (typeof Lang !== 'undefined' && Lang.getLang() === 'id' ? 'Giliran: ' : 'Turn: ') + player.name;
    const roundEl = document.getElementById('hud-round');
    if(roundEl) roundEl.textContent = 'R' + (gameState.roundCount || 1);
    const turnNumEl = document.getElementById('hud-turn-number');
    if(turnNumEl) turnNumEl.textContent = 'T' + (gameState.turnNumber || 1);
    const moneyEl = document.getElementById('hud-money');
    if(moneyEl) {
      const myId = typeof Network !== 'undefined' ? Network.getPlayerId() : (player ? player.id : null);
      const myPlayer = gameState.players.find(p => p.id === myId) || player;
      const cash = myPlayer ? (myPlayer.money || 0) : 0;
    ...
    if(weatherEl && typeof Weather !== 'undefined' && Weather.getWeatherInfo) {
      ...
      weatherEl.title = (typeof Lang !== 'undefined' && Lang.getLang() === 'id') 
        ? (wInfo ? ('Cuaca: ' + wInfo.name_id + ' — ' + (wInfo.desc_id || '')) : 'Cuaca Normal')
        : (wInfo ? ('Weather: ' + wInfo.name_en + ' — ' + (wInfo.desc_en || '')) : 'Sunny');
    }
    const econEl = document.getElementById('hud-economy');
    if(econEl) {
      if(gameState.economy && gameState.economy.currentEvent) {
        const evId = gameState.economy.currentEvent;
        const shortNames = {
          bullMarket: '📈 Bull',
          crash: '📉 Crash',
          inflation: '💸 Inflasi',
          constructionBoom: '🏗️ Boom',
          pandemic: '☣️ Pandemi',
          techRevolution: '🤖 Tech',
          bailout: '🏛️ Bailout',
          fire: '🔥 Kebakaran',
          goldRush: '🪙 Emas',
          disaster: '🌋 Bencana',
          cyberWarfare: '🛡️ Siber',
          cryptoBoom: '💎 Kripto',
          taxHoliday: '📜 Tax Free',
          megaMerger: '🚄 Merger',
          quantumGlitch: '🌌 Glitch',
          hyperInflation: '📈 Hiper'
        };
        const shortLabel = shortNames[evId] || '🌐 Event';
        econEl.textContent = shortLabel;
        const fullDesc = (typeof Lang !== 'undefined') ? Lang.t('evt_' + evId) : shortLabel;
        econEl.title = fullDesc;
        econEl.style.background = 'rgba(236, 72, 153, 0.25)';
        econEl.style.borderColor = 'rgba(236, 72, 153, 0.6)';
        econEl.style.color = '#f472b6';
      } else {
        econEl.textContent = '🌐 Normal';
        econEl.title = 'Kondisi Ekonomi Normal (Tidak ada event aktif)';
        econEl.style.background = 'rgba(79, 140, 255, 0.12)';
        econEl.style.borderColor = 'rgba(79, 140, 255, 0.3)';
        econEl.style.color = '#94a3b8';
      }
    }
  }
  ...
  function showCardDrawnModal(player, card, type) {
    if(!card) return;
    const lang = typeof Lang !== 'undefined' ? Lang.getLang() : 'id';
    const cardTitle = (lang === 'id' ? card.name_id : card.name_en) || card.name_en || '';
    const cardDesc = (lang === 'id' ? card.desc_id : card.desc_en) || card.desc_en || '';
    const isChance = type === 'chance';
    const typeLabel = isChance ? (lang === 'id' ? '❓ KARTU KESEMPATAN (CHANCE)' : '❓ CHANCE CARD') : (lang === 'id' ? '📦 PETI SIBER (CYBER CHEST)' : '📦 CYBER CHEST');
    const badgeColor = isChance ? '#f59e0b' : '#38bdf8';
    const playerName = player ? (player.name || 'Player') : 'Player';

    const body = document.querySelector('#modal-event .modal-body');
    const titleEl = document.querySelector('#modal-event .modal-title');
    if(titleEl) titleEl.innerHTML = `<span style="color:${badgeColor}">${typeLabel}</span>`;
    
    if(body) {
      body.innerHTML = `
        <div style="text-align:center;padding:1rem 0.5rem;">
          <div style="font-size:3.2rem;margin-bottom:0.5rem;animation:bounce 0.6s ease;">${isChance ? '❓' : '📦'}</div>
          <div style="font-size:0.85rem;color:${(player && player.color) ? player.color : '#38bdf8'};font-weight:bold;margin-bottom:0.4rem;">
            👤 ${playerName}
          </div>
          <h3 style="color:${badgeColor};font-size:1.25rem;font-weight:800;margin-bottom:0.6rem;">
            ${cardTitle}
          </h3>
          <p style="color:#e2e8f0;font-size:0.9rem;line-height:1.55;background:rgba(0,0,0,0.35);padding:0.75rem;border-radius:8px;border:1px solid rgba(255,255,255,0.08);max-width:380px;margin:0 auto 1.25rem auto;">
            ${cardDesc}
          </p>
          <button class="menu-btn btn-primary" onclick="UI.hideModal('event');" style="padding:0.6rem 2.2rem;font-size:0.9rem;">
            ${lang === 'id' ? 'LANJUTKAN / OK' : 'CONTINUE / OK'}
          </button>
        </div>
      `;
      showModal('event');
    ...
    showToast(`${playerName}: ${cardTitle}`, isChance ? 'warning' : 'info', 4500);
  }
  ...
  if(typeof Events !== 'undefined') {
    ...
    Events.on('cardDrawn', (data) => { if(data && data.card) showCardDrawnModal(data.player, data.card, data.type); });
