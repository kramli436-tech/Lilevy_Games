...
class MonopolyUI {
      ...
      setTimeout(() => {
        ...
        cubes.forEach(c => {
          if (c) {
            ...
            if (isDouble) c.classList.add('ring-4', 'ring-amber-400', 'shadow-lg');
            else c.classList.remove('ring-4', 'ring-amber-400', 'shadow-lg');
          }
        });
      }, 500);
    };
  ...
  // Render Langkah Token dengan Efek Visual Berjalan & Ripple
  renderStepMove(player, currentPos) {
    ...
    const tileEl = document.getElementById(`tile-${currentPos}`);
    if (tileEl) {
    ...
    engine.players.forEach(p => {
      if (!p.isBankrupt) {
        ...
        if (container) {
          ...
          badge.className = `player-token-badge ${isWalking ? 'token-walking' : ''}`;
  ...
  showBuyPrompt(player, tile) {
    ...
    // Render Tabel Struktur Tarif Sewa
    const rentContainer = document.getElementById('buy-prop-rent-structure');
    if (rentContainer) {
      rentContainer.innerHTML = this.renderRentStructureTable(tile, prop);
    }
    ...
    btnSkip.textContent = 'Lelang Terbuka 🔨';
