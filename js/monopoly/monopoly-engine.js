...
class MonopolyEngine {
  ...
  handleTileLanding(player, tileId) {
    ...
    // Mendarat di Parkir Bebas / Istirahat -> Kasino Roda Keberuntungan
  ...
  calculateRent(tileId) {
    ...
    if (window.monopolyEconomy) {
      rent = window.monopolyEconomy.getModifiedRent(tile, rent);
    }
  ...
  handleAIBuyDecision(player, tile) {
    const buffer = player.aiLevel === 'expert' ? 1500000 : 1000000;
    ...
    if (canAfford || (isStrategic && player.money >= tile.price)) {
      ...
      this.log(`🤖 ${player.name} melewatkan pembelian [${tile.name}]. Membuka Lelang Publik!`);
      this.phase = 'AUCTION';
