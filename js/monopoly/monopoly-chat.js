...
class MonopolyChat {
  ...
  sendMessage(senderName, text, isAI = false, token = '👤', skipBroadcast = false) {
    ...
    // Broadcast pesan ke seluruh pemain di Room via RoomNetwork
    if (!skipBroadcast && window.roomNetwork) {
      window.roomNetwork.sendChat({
        senderName,
        text: text.trim(),
        avatar: token
      });
  ...
  receiveRemoteMessage(chat) {
    if (!chat || !chat.text) return;
    this.sendMessage(chat.senderName || 'Pemain', chat.text, false, chat.avatar || '💬', true);
  }
