const Dice = (function() {
  ...
  function roll(player) {
    let dice1 = (typeof WasmEngine !== 'undefined' && WasmEngine.isReady()) ? WasmEngine.fastRng(1, 6) : Math.floor(Math.random() * 6) + 1;
    let dice2 = (typeof WasmEngine !== 'undefined' && WasmEngine.isReady()) ? WasmEngine.fastRng(1, 6) : Math.floor(Math.random() * 6) + 1;
    if (player && player.loadedDice) {
      dice1 = 6;
      dice2 = (typeof WasmEngine !== 'undefined' && WasmEngine.isReady()) ? WasmEngine.fastRng(5, 6) : (5 + Math.floor(Math.random() * 2)); // 5 or 6 (Total: 11 or 12)
      player.loadedDice = false;
    }
    lastRoll = {
      dice1, dice2,
      values: [dice1, dice2],
  ...
  function animateRoll(callback, player) {
    if (isRolling) {
      isRolling = false;
    ...
    if (typeof GameAudio !== 'undefined' && GameAudio.play) {
      try { GameAudio.play('dice'); } catch(e) {}
    }

    const result = roll(player);
    ...
    setTimeout(() => {
      ...
      if (callback) {
        try {
          ...
          console.error('Error in dice callback:', e);
        }
      }
    }, 1150);
