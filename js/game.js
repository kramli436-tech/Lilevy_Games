const Game = (function() {
  ...
  function executeMove(player, diceResult) {
    ...
    const totalSteps = (diceResult && diceResult.total) ? diceResult.total : 1;
    const direction = (player.reverseTurns && player.reverseTurns > 0) ? -1 : 1;
    let stepCount = 0;
    
    state.turnPhase = 'MOVING';
    Events.emit('stateUpdated', state);
    if(typeof UI !== 'undefined') UI.updateActionButtons(state);
    ...
    const stepInterval = setInterval(() => {
      ...
      if (typeof GameAudio !== 'undefined' && GameAudio.play) GameAudio.play('move');
  ...
  function endTurn() {
    if(!state) return;
    if(state.turnPhase !== 'ACTION' && state.turnPhase !== 'END' && state.turnPhase !== 'LAND') return;
    const player = getCurrentPlayer();
    if(!player) return;
